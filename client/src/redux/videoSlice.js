  ...
  reducers: {
    ...
    likeVideo: (state, action) => {
      if (!state.video.likes.includes(action.payload)) {
        ...
        state.video.dislikes.splice(state.video.dislikes
    ...
    disLikeVideo: (state, action) => {
      if (!state.video.dislikes.includes(action.payload)) {
        ...
        state.video.likes.splice(state.video.dislikes
          .findIndex((userId) => userId === action.payload), 1)
      }
    }
  }
...
export const {VideoStart, VideoSuccess, VideoFailure, likeVideo, disLikeVideo} = videoSlice.actions
